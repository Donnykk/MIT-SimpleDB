...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        //判断是否有入口结点
        if (!it.hasNext()) {
            throw new DbException("No entry!");
        }
        //f为null则返回左孩子结点
        if (f == null) {
            entry = it.next();
    ...
    BTreeLeafPage findLeafPage(TransactionId tid, BTreePageId pid, Field f)
            throws DbException, TransactionAbortedException {
        return findLeafPage(tid, new HashMap<PageId, Page>(), pid, Permissions.READ_ONLY, f);
    }
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        // page and moving half of the tuples to the new page.  Copy the mid_entry key up
        // into the parent page, and recursively split the parent as needed to accommodate
        // the new entry.  getParentWithEmptySlots() will be useful here.  Don't forget to update
        // the sibling pointers of all the affected leaf pages.  Return the page into which a
        // tuple with the given key field should be inserted.
        BTreeLeafPage newLp = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
        ...
        for (int i = 0; i < numTuples / 2; i++) {
            ...
            newLp.insertTuple(tuple);
        }

        //更新父结点
        Field newField = newLp.iterator().next().getField(keyField);
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), newField);
        page.setParentId(parent.getId());
        newLp.setParentId(parent.getId());
        BTreeEntry entry = new BTreeEntry(newField, page.getId(), newLp.getId());
        parent.insertEntry(entry);

        //更新兄弟结点
        newLp.setLeftSiblingId(page.getId());
        newLp.setRightSiblingId(page.getRightSiblingId());
        page.setRightSiblingId(newLp.getId());

        if (newLp.getRightSiblingId() != null) {
            BTreeLeafPage rightPage = (BTreeLeafPage) getPage(tid, dirtypages, newLp.getRightSiblingId(), Permissions.READ_WRITE);
            rightPage.setLeftSiblingId(newLp.getId());
            dirtypages.put(rightPage.getId(), rightPage);
        }

        //更新脏页
        dirtypages.put(page.getId(), page);
        dirtypages.put(newLp.getId(), newLp);
        dirtypages.put(parent.getId(), parent);
        if (field.compare(Op.GREATER_THAN_OR_EQ, newField)) {
            return newLp;
        }
        return page;
    }
     ...
     * the mid_entry key in the original internal page being split (this key is "pushed up" to the parent).
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // page and moving half of the entries to the new page.  Push the mid_entry key up
        // into the parent page, and recursively split the parent as needed to accommodate
        // the new entry.  getParentWithEmptySlots() will be useful here.  Don't forget to update
        // the parent pointers of all the children moving to the new page.  updateParentPointers()
        // will be useful here.  Return the page into which an entry with the given key field
        // should be inserted.
        BTreeInternalPage newIp = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
        ...
        for (int i = 0; i < numEntry / 2; i++) {
            ...
            newIp.insertEntry(entry);
            //当entry被添加到newIp之后recordId被更改,所以只能先删除再插入到新的Page中!!!
        }
        BTreeEntry mid_entry = reverseIt.next();
        page.deleteKeyAndRightChild(mid_entry);
        updateParentPointers(tid, dirtypages, newIp);
        //将中间entry挤入父结点
        BTreeInternalPage parent = getParentWithEmptySlots
                (tid, dirtypages, page.getParentId(), mid_entry.getKey());
        mid_entry.setLeftChild(page.getId());
        mid_entry.setRightChild(newIp.getId());
        parent.insertEntry(mid_entry);
        page.setParentId(parent.getId());
        newIp.setParentId(parent.getId());
        //写入脏页
        dirtypages.put(page.getId(), page);
        dirtypages.put(newIp.getId(), newIp);
        dirtypages.put(parent.getId(), parent);
        if (field.compare(Op.GREATER_THAN_OR_EQ, mid_entry.getKey())) {
            return newIp;
        }
        return page;
    }
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int numTuple = page.getNumTuples();
        int numTuple_sibling = sibling.getNumTuples();
        int standardNum = (numTuple + numTuple_sibling) / 2;
        Iterator<Tuple> it;
        if (isRightSibling) {
            it = sibling.iterator(); //若从右兄弟窃取，则用正向迭代器
        } else {
            it = sibling.reverseIterator(); //若从左兄弟窃取，用反向迭代器
        }
        while (numTuple < standardNum) {
            Tuple tuple = it.next();
            sibling.deleteTuple(tuple);
            page.insertTuple(tuple);
            numTuple++;
        }
        //将entry的key更改为page和其兄弟节点key的中间值
        Tuple mid = it.next();
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // some code goes here
        // Move some of the entries from
        // left sibling to the page so
        // that the entries are evenly distributed. Be sure to update
        // the corresponding parent entry. Be sure to update the parent
        // pointers of all children in the entries that were moved.
        int num_entry = page.getNumEntries();
        int num_siblingEntry = leftSibling.getNumEntries();
        int standardNum = (num_entry + num_siblingEntry) / 2;
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // pointers of all children in the entries that were moved.
        int num_entry = page.getNumEntries();
        int num_siblingEntry = rightSibling.getNumEntries();
        int standardNum = (num_entry + num_siblingEntry) / 2;
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // Delete the entry in the parent corresponding to the two pages that are merging -
        // deleteParentEntry() will be useful here
        BTreeEntry mid_entry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(),
                rightPage.iterator().next().getLeftChild());
        leftPage.insertEntry(mid_entry); //将父结点中的page插入左结点
